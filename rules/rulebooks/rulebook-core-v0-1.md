...
1. raw source 文本层
2. source record 记录层
raw source 属于 raw-sources 目录。
source record 属于 wiki 或模板所定义的记录层对象。
